def gneed(prelimg, targetg):
    ...
    returns:
    -a tuple required midterm and final grades.
    """

    prelimweight = 0.20
    midtermweight = 0.30
    finalweight = 0.50
    # target formula: targetgrade = (prelimweight * prelimgrade) + (midtermweight * midtermgrade) + (finalweight * fianlgrade)
    # rearranged formula: (targetgrade - prelimweight * midtermgrade) = (midtermweight * midtermgrade) + finalweight * finalgrade) 
    ...
    if reqfinalgrade > 100:
        ...
        reqmidtermgrade = (remaininggrade - (finalweight * 100)) / finalweight

        if reqfinalgrade < 0 or reqmidtermgrade > 100:
